    ...
    if let Ok((mut transform, mut projection)) = cameraquery.single_mut() {
        ...
        if let Projection::Orthographic(projection2d) = &mut *projection {
            ...
            if c1 || c2 {
                ...
                let mut event = ESetLod {
                    parameter: layer.lod_level,
                    center: layer.map_center,
                };
                if c1 {
                    event.parameter = (lv.max(0).min(LOD_IDX.len() as i8 - 1)) as u8;
                }
                if c2 {
                    event.center = IVec2::new(x, y);
                }
                debug!(
                    "lod:{}; cur:{};pre:{},c1:{},c2:{}",
                    event.parameter, cur, pre, c1, c2
                );
...
pub fn petcontrol(
    ...
    mut goballocation: ResMut<res::sync::RGobalLocation>,
    input: Res<ButtonInput<KeyCode>>,
    ...
    if let Ok((camera, global_transform, mut transformca)) = camera_query.single_mut() {
        ...
        let mut gobalindex = goballocation.index;
        ...
        if keyinput {
            if let Ok((mut behavior, transform)) = petquery.get_mut(petselect.0.unwrap()) {
                ...
                gobalindex = behavior.gobalpos;
            }
        } else {
            let (translation, pos) =
                super::get_location(transformca.translation, goballocation.index);
            transformca.translation = translation;
            gobalindex = pos;
        };
        if goballocation.pos != transformca.translation {
            goballocation.pos = transformca.translation;
            goballocation.index = gobalindex;
