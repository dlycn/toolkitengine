...
pub fn e_create_pet(
    ...
    pet_config.insert(5000, PetConfig{
        texture_size:[256,224],
        pet_name:"圣灵谱尼".to_string(),
        pet_indice:26,
    });
    pet_config.insert(3414, PetConfig{
    ...
    for (param, func) in event.idnum.iter() {
        let id = param[0];
        let num = param[1];
        let size = pet_config.get(&id).unwrap().texture_size;
        let name = pet_config.get(&id).unwrap().pet_name.clone();
        let indice = pet_config.get(&id).unwrap().pet_indice;
        ...
        for i in 0..num {
            ...
            respet.0 = Some(commands.spawn_scene(pet).id());
        }
    }
}
